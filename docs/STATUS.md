...
> T-304 +24 → 308；T-401 +16 → 324；T-402 +11 → 335；T-403 +14 → 349；T-404 +11 → 360；T-501 +12 → 372；T-502 +9 → 381；T-503 +12 → 393；T-504 +14 → 407；T-505 +10 → 417；T-105 +4 → **421**。
>
> ⚠️ **无 Node 的机器**：`MermaidGenTest` 的 3 个官方解析器用例会**显式跳过**并打印启用命令，
> 此时是 **418 通过 + 3 跳过**，不是失败。启用：`cd tools/mermaid-verify && npm install`。
