...
**阶段 0 / 1A / 2 / 3 / 4 已完成**（阶段 3 的 T-305 因 Redis 挂起）；阶段 5 完成 2/5。
`mvn clean verify` → **7 模块全绿，381 个测试，0 失败 0 跳过**。

| 快速数字 | 值 |
|---|---|
| 完成任务卡 | **24 / 62**（另有 14 张因缺 Docker 挂起） |
| 测试 | **381 个，0 失败 0 跳过**（common 12 / gateway 1 / project-resource 210 / code-analysis 153 / agent-orchestration 4 / evaluation-export 1） |
| 主代码 | 79 个 Java 文件 / 8153 行 |
| 测试代码 | 35 个 Java 文件 / 7447 行（测试与主代码量级相当） |
| 提交 | 18 个，全部已推送 |
| 文档 | 8 份（`CLAUDE.md` + `docs/*.md`）+ 1 个测试工具（`tools/mermaid-verify/`） |

> ⚠️ **基线数字更正**：此前文档记的基线是 283，长期实测为 **284**（少算 1 个）。
> T-304 +24 → 308；T-401 +16 → 324；T-402 +11 → 335；T-403 +14 → 349；T-404 +11 → 360；T-501 +12 → 372；T-502 +9 → **381**。
>
> ⚠️ **无 Node 的机器**：`MermaidGenTest` 的 3 个官方解析器用例会**显式跳过**并打印启用命令，
> 此时是 **378 通过 + 3 跳过**，不是失败。启用：`cd tools/mermaid-verify && npm install`。
...
| T-502 | **pom 依赖冲突检测** | 同一 pom 多版本**高危** / 跨模块不一致**中危且措辞不含「冲突」** / 重复声明低危 / 坐标无效；冲突路径给出每一处声明的 `文件:行号`；负向断言干净模块零产出；阶段 5 门禁 **21 个测试全绿** |
...
| 卡号 | 任务 | 所属阶段 |
|---|---|---|
| **T-503** | **`requirements.txt` 依赖解析与冲突检测** ← **下一张** | 阶段 5 |
| T-504~T-505 | 架构隐患 / 风险分级与入库 | 阶段 5 |
...
```bash
# 1. 跑基线，确认起点正确（预期：7 模块 SUCCESS，381 测试，0 失败 0 跳过）
#    没有 Node 的机器上是 378 通过 + 3 跳过（MermaidGenTest 的官方解析器用例显式跳过）
cd D:/CodeWisdom
mvn clean verify

# 2. 下一张卡
#    T-503 —— requirements.txt 依赖解析与冲突检测（Python 侧）
#    涉及模块：codewisdom-code-analysis
#    照抄 T-502 的形状：读取器（RequirementsReader）+ 冲突分析器（RequirementsConflictAnalyzer），
#    产出统一走 AuditIssue（category=DEPENDENCY）
#    ⚠️ Python 侧与 Maven 的冲突语义不同，别照搬结论：
#      · 同一文件内同一包写两行不同版本 → 后者生效（pip 行为），与 Maven 类似
#      · 多文件（requirements.txt / requirements-dev.txt）之间版本不一致 → 同样是「声明不一致」
#      · Python 的包名规范是 PEP 503（小写 + 下划线转连字符），T-403 已有归一化实现可参考
#    验收：构造样例能检出冲突并给出路径
#    测试：mvn -q -pl codewisdom-code-analysis -am -Dtest=ReqConflictTest -Dsurefire.failIfNoSpecifiedTests=false test
```

然后对我说：**「先读 CLAUDE.md、docs/progress.md、docs/tasks.md。当前任务：T-503」** 即可。
