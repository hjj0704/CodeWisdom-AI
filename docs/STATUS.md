...
**阶段 0 / 1A / 2 / 3 / 4 已完成**（阶段 3 的 T-305 因 Redis 挂起）。
`mvn clean verify` → **7 模块全绿，360 个测试，0 失败 0 跳过**。

| 快速数字 | 值 |
|---|---|
| 完成任务卡 | **22 / 62**（另有 14 张因缺 Docker 挂起） |
| 测试 | **360 个，0 失败 0 跳过**（common 12 / gateway 1 / project-resource 210 / code-analysis 132 / agent-orchestration 4 / evaluation-export 1） |
| 主代码 | 69 个 Java 文件 / 7033 行 |
| 测试代码 | 33 个 Java 文件 / 6681 行（测试与主代码量级相当） |
| 提交 | 16 个，全部已推送 |
| 文档 | 8 份（`CLAUDE.md` + `docs/*.md`）+ 1 个测试工具（`tools/mermaid-verify/`） |

> ⚠️ **基线数字更正**：此前文档记的基线是 283，长期实测为 **284**（少算 1 个）。
> T-304 +24 → 308；T-401 +16 → 324；T-402 +11 → 335；T-403 +14 → 349；T-404 +11 → **360**。
>
> ⚠️ **无 Node 的机器**：`MermaidGenTest` 的 3 个官方解析器用例会**显式跳过**并打印启用命令，
> 此时是 **357 通过 + 3 跳过**，不是失败。启用：`cd tools/mermaid-verify && npm install`。
...
| 卡号 | 任务 | 所属阶段 |
|---|---|---|
| **T-501** | **审计规则引擎 + 规则注册机制** ← **下一张** | 阶段 5 |
| T-801~T-802 | 运行能力判定 / 部署指引生成（**不依赖中间件，可随时插入**） | 阶段 8 |
| T-502~T-505 | pom 依赖冲突 / requirements 冲突 / 架构隐患 / 风险分级 | 阶段 5 |
...
```bash
# 1. 跑基线，确认起点正确（预期：7 模块 SUCCESS，360 测试，0 失败 0 跳过）
#    没有 Node 的机器上是 357 通过 + 3 跳过（MermaidGenTest 的官方解析器用例显式跳过）
cd D:/CodeWisdom
mvn clean verify

# 2. 下一张卡
#    T-501 —— 审计规则引擎 + 规则注册机制（阶段 5 首卡）
#    涉及模块：codewisdom-code-analysis
#    现成输入：LayerReport（分层与 mixedPackages）、CycleReport（循环依赖）、
#             TechStackReport（技术栈）—— T-504「架构隐患」本质是把这几份报告
#             翻译成统一问题模型，T-501 先把规则引擎与问题模型立起来
#    验收：空指针/未捕获异常/硬编码/死代码规则可插拔
#    测试：mvn -q -pl codewisdom-code-analysis -am -Dtest=RuleEngineTest -Dsurefire.failIfNoSpecifiedTests=false test
```

然后对我说：**「先读 CLAUDE.md、docs/progress.md、docs/tasks.md。当前任务：T-501」** 即可。
