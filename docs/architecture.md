...
导入的仓库与压缩包都是**不可信输入**，按以下顺序处理，每一层都在建业务记录之前尽可能挡掉：

```text
上传/地址
  → ① 廉价前置校验（空文件、扩展名、URL 协议/端口/白名单）   ← 失败：不创建项目
  → ② 结构预检（ZIP 魔数、条目名合法性）                     ← 失败：不创建项目
  → ③ 创建 Project + ImportTask（此后才有业务记录）
  → ④ 昂贵且有副作用的一步（克隆 / 解压，含炸弹熔断）        ← 失败：留下 FAILED 记录（可审计）
  → ⑤ 扫描建树 + 落库 + 置终态
```

**为什么 ① ② 要在建记录之前**：攻击者若能靠畸形输入批量制造垃圾项目记录，
本身就是一种资源耗尽手段。

**为什么 ④ 的失败要留记录**：压缩炸弹无法预判（条目声明的 size 会撒谎），
只能边写边熔断；这类输入留下 FAILED 记录是期望行为，便于安全审计。

**Windows 平台注意**：Git 写出的 pack 文件带只读属性，任何递归删除
（工作区重建、`mvn clean`）都必须先 `setWritable(true)`，否则
`AccessDeniedException` 会让「第二次导入」失败、让构建中断。
