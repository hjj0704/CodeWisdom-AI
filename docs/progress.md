...
- **当前任务卡**：T-301（阶段 3 起点：Tree-Sitter 解析器封装 + 语言注册表）
- **已完成**：T-000 文档初始化、T-001 `git init`、T-002 工程骨架、T-004~T-007 四项冒烟、
  T-101/T-102/T-106（阶段 1A）、T-201 持久化、T-202 Git 导入、T-203 ZIP 导入、**T-204 文件树与统计**
- **构建基线**：`mvn clean verify` → 7 模块全绿，**231 个测试**，0 失败 0 跳过
