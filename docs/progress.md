...
2. **下一张卡：T-501 —— 审计规则引擎 + 规则注册机制**
   （阶段 4 已全部收尾；另：阶段 8 的 T-801/T-802 不依赖中间件，也可随时插入）
3. 起手前先跑一次基线：`mvn clean verify`
   预期：**7 模块 SUCCESS，360 个测试，0 失败 0 跳过**
   （若机器上没有 Node，`MermaidGenTest` 的 3 个官方解析器用例会**显式跳过**并打印启用办法，
   此时是 **357 通过 + 3 跳过**，不是失败。）
4. 工作模式：自主执行，每张卡完成后用中文详细提交信息 commit + `git push origin main`。
5. `github.com` 本机不可达（R-08），需要真实仓库验证时用 **Gitee**。

**最新提交**：`feat(code-analysis): 模块循环依赖检测（T-404）`（与 `origin/main` 一致）

**已解锁**：**阶段 4 架构逆向 4/4 全部完成**。阶段 5 可直接复用阶段 4 的全部产物：
`LayerReport`（分层与分层混乱）、`CycleReport`（循环依赖）、`TechStackReport`（技术栈），
T-504「架构隐患」规则本质上是把这几份报告翻译成统一问题模型。
...
| 阶段 4 | 架构逆向 | ✅ **已完成** | 4 / 4 |
| 阶段 5 | 缺陷与依赖审计 | ⬜ 未开始（下一张卡 T-501） | 0 / 5 |
| 阶段 6 | 文档注释生成 | ⬜ 未开始 | 0 / 4 |
...
- **当前任务卡**：T-501（审计规则引擎 + 规则注册机制）——阶段 5 首卡
- **已完成**：T-000 文档初始化、T-001 `git init`、T-002 工程骨架、T-004~T-007 四项冒烟、
  ...
  T-401 包结构与分层识别、T-402 Mermaid 架构图生成、T-403 技术栈识别、**T-404 模块循环依赖检测**
- **构建基线**：`mvn clean verify` → 7 模块全绿，**360 个测试**，0 失败 0 跳过
  （各模块：common 12 / gateway 1 / project-resource 210 / code-analysis 132 /
  agent-orchestration 4 / evaluation-export 1）
  ⚠️ 无 Node 的机器上 `MermaidGenTest` 的 3 个官方解析器用例会显式跳过 → 357 通过 + 3 跳过
