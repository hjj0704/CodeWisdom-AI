...
2. **下一张卡：T-502 —— `pom.xml` 依赖冲突检测**
3. 起手前先跑一次基线：`mvn clean verify`
   预期：**7 模块 SUCCESS，372 个测试，0 失败 0 跳过**
   （若机器上没有 Node，`MermaidGenTest` 的 3 个官方解析器用例会**显式跳过**并打印启用办法，
   此时是 **369 通过 + 3 跳过**，不是失败。）
4. 工作模式：自主执行，每张卡完成后用中文详细提交信息 commit + `git push origin main`。
5. `github.com` 本机不可达（R-08），需要真实仓库验证时用 **Gitee**。

**最新提交**：`feat(code-analysis): 审计规则引擎与规则注册机制（T-501）`（与 `origin/main` 一致）

**已解锁**：阶段 5 已开工。`AuditIssue`（统一问题模型）+ `AuditRule`/`AuditEngine`（可插拔引擎）
已就位，T-502~T-505 本质上是往里填规则：
T-502/T-503 填依赖冲突规则，T-504 把阶段 4 的三份报告（`LayerReport`/`CycleReport`/`TechStackReport`）
翻译成 `AuditIssue`，T-505 做风险分级与入库。
...
- **构建基线**：`mvn clean verify` → 7 模块全绿，**372 个测试**，0 失败 0 跳过
  （各模块：common 12 / gateway 1 / project-resource 210 / code-analysis 144 /
  agent-orchestration 4 / evaluation-export 1）
  ⚠️ 无 Node 的机器上 `MermaidGenTest` 的 3 个官方解析器用例会显式跳过 → 369 通过 + 3 跳过
