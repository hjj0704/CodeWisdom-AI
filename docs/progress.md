...
2. **下一张卡：T-504 —— 架构隐患规则（循环依赖 / 分层混乱 / 职责不单一）**
3. 起手前先跑一次基线：`mvn clean verify`
   预期：**7 模块 SUCCESS，393 个测试，0 失败 0 跳过**
   （若机器上没有 Node，`MermaidGenTest` 的 3 个官方解析器用例会**显式跳过**并打印启用办法，
   此时是 **390 通过 + 3 跳过**，不是失败。）
...
**已解锁**：阶段 5 已完成 3/5。Java 代码规则（T-501）与两侧依赖冲突（T-502/T-503）都已就位。
**T-504 是阶段 4 与阶段 5 的接合点**：阶段 4 已经产出 `LayerReport`（分层与 `mixedPackages()`）、
`CycleReport`（循环依赖）、`TechStackReport`（技术栈），那三份报告早已是结构化数据，
T-504 本质上是把它们**翻译成 `AuditIssue`**——不需要重新解析源码。
T-505 再做风险分级汇总与入库。
...
- **当前任务卡**：T-504（架构隐患规则：循环依赖 / 分层混乱 / 职责不单一）
- **已完成**：T-000 文档初始化、T-001 `git init`、T-002 工程骨架、T-004~T-007 四项冒烟、
  ...
  T-502 pom 依赖冲突检测、**T-503 requirements 依赖冲突检测**
- **构建基线**：`mvn clean verify` → 7 模块全绿，**393 个测试**，0 失败 0 跳过
  （各模块：common 12 / gateway 1 / project-resource 210 / code-analysis 165 /
  agent-orchestration 4 / evaluation-export 1）
  ⚠️ 无 Node 的机器上 `MermaidGenTest` 的 3 个官方解析器用例会显式跳过 → 390 通过 + 3 跳过
