...
2. **下一张卡：T-401 —— 包结构/分层识别（controller/service/mapper/...）**
   （阶段 3 的非挂起卡已全部完成；T-305 依赖 Redis，挂起）
3. 起手前先跑一次基线：`mvn clean verify`
   预期：**7 模块 SUCCESS，308 个测试，0 失败 0 跳过**
...
**已解锁**：阶段 2、阶段 3 非挂起卡全部完成，阶段 4 可随时开工。
T-304 产出的调用图（`methodEdges()` / `typeEdges()`）正是 T-401 分层识别与
T-404 循环依赖检测的输入，阶段 4 可以直接接上。
...
- **构建基线**：`mvn clean verify` → 7 模块全绿，**308 个测试**，0 失败 0 跳过
  （各模块：common 12 / gateway 1 / project-resource 210 / code-analysis 80 /
