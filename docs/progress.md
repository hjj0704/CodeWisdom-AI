...
2. **下一张卡：T-402 —— Mermaid 架构图生成（模块图/依赖拓扑图）**
3. 起手前先跑一次基线：`mvn clean verify`
   预期：**7 模块 SUCCESS，324 个测试，0 失败 0 跳过**
4. 工作模式：自主执行，每张卡完成后用中文详细提交信息 commit + `git push origin main`。
5. `github.com` 本机不可达（R-08），需要真实仓库验证时用 **Gitee**。

**最新提交**：`feat(code-analysis): 包结构与分层识别（T-401）`（与 `origin/main` 一致）

**已解锁**：阶段 2、阶段 3 非挂起卡全部完成；阶段 4 已开工（T-401 完成）。
T-402 可直接用上两份现成产物：`LayerReport`（分层与包结构）与
`CallGraph.typeEdges()`（类型级调用边，已去自环）。
...
- **已完成**：T-000 文档初始化、T-001 `git init`、T-002 工程骨架、T-004~T-007 四项冒烟、
  ...
  **T-401 包结构与分层识别**
- **构建基线**：`mvn clean verify` → 7 模块全绿，**324 个测试**，0 失败 0 跳过
  （各模块：common 12 / gateway 1 / project-resource 210 / code-analysis 96 /
