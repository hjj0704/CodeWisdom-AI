...
const hasTarget = computed(() => !!current.value?.target)
...
const shadePanels = computed(() => {
  ...
  return {
    top: { top: 0, left: 0, width: w, height: t },
    left: { top: t, left: 0, width: l, height: b - t },
    right: { top: t, left: r, width: w - r, height: b - t },
    bottom: { top: b, left: 0, width: w, height: h - b },
  }
})
...
<template>
  <Teleport to="body">
    <div v-if="visible" class="tour-root">
      ...
      <div class="tour-card glass-panel" :class="{ 'tour-card--center': !hasTarget }" :style="cardStyle">
...
.tour-spotlight-ring {
  ...
  border-radius: 12px;
  box-shadow: 0 0 0 4px rgba(34, 211, 238, 0.2), 0 0 24px rgba(34, 211, 238, 0.35);
  pointer-events: none;
  z-index: 1;
...
@keyframes tour-pulse {
  ...
  50% {
    box-shadow: 0 0 0 6px rgba(34, 211, 238, 0.35), 0 0 32px rgba(34, 211, 238, 0.5);
  }
}
...
.tour-card {
  ...
  animation: tour-card-glow 2.4s ease-in-out infinite;
}
...
.illus-workbench {
  background: linear-gradient(90deg, rgba(34, 211, 238, 0.1) 28%, transparent 28%),
